"""``orin cli`` — noun grouping CLI-surface introspection.
...
action-verbs under ``cli`` today, but ``orin cli overview`` describes the CLI
surface (distinct from the global ``overview``, which describes the agent).
"""
...
from jetson_orin.cli._commands.overview import cli_sections, emit_overview
...
def _no_verb(args: argparse.Namespace) -> int:
    # `orin cli` with no sub-verb prints the noun's overview.
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "cli",
        help="CLI-surface introspection (see 'orin cli overview').",
    )
    ...
    ov = noun_sub.add_parser("overview", help="Describe the orin CLI surface.")
