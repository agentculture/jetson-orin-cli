...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope jetson-orin-cli
...
scope:** the default personal scope is **private** (`--scope jetson-orin-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `jetson-orin-cli`). Private records are served only to a recall in the same
