...
jetson-orin-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. jetson-orin-cli
...
#   - consumer-identifying prose: `guildmaster` → `jetson-orin-cli` (NOT
...
If a re-sync would lose a jetson-orin-cli adaptation, lift the change
...
#     → `which jetson-orin-cli vendors from guildmaster`
